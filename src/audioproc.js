 ...
 *
 * The full game plays recorded samples instead (src/audio.js) — this stays as
 * the base class for two reasons: the playable ads inline every byte they ship,
 * so a megabyte of mp3 is not on the table, and a cue whose sample hasn't
 * finished decoding falls through to the synth rather than going silent.
 */
export class ProcAudio {
  constructor() {
    this.ctx = null;
    this.master = null;
  ...
  /**
   * Lapping water + breeze bed. Both layers meet at one unmodulated bed gain on
   * the way to the master, which is what lets the sample build fade the whole
   * thing out cleanly — the per-layer gains have LFOs writing to them, so
   * ramping those to zero would leave the wobble audible underneath.
   */
  _startLakeAmbience() {
    this._ambienceBed = this.ctx.createGain();
    this._ambienceBed.connect(this.master);
    ...
    src.connect(filt).connect(g).connect(this._ambienceBed);
    src.start();
    lfo.start();
    ...
    src2.connect(filt2).connect(g2).connect(this._ambienceBed);
    src2.start();
    lfo2.start();
    this._ambienceSources = [src, src2, lfo, lfo2];
  }
  ...
  /** no music without assets; the sample build overrides these */
  playMusic() {}
  stopMusic() {}

  /**
   * Positional audio is the sample build's job (src/audio.js). Placing a cue
   * means routing it through a panner and a lowpass, and every synth cue here
   * is three or four oscillators that would each need threading through the
   * same chain — a lot of plumbing for the ads, which are five short holes
   * where the rivals are never far away. They hear a flat mix, and the position
   * every caller passes lands here and is ignored.
   */
  setListener() {}
  ...
  /**
   * Underside of a bridge deck or a cave roof. Timber is a hollow knock with a
   * bit of ring left in it; rock is the same hit with the ring taken out and
   * dropped an octave, so the two read apart without a second sample.
   */
  headKnock(kind = "bridge") {
    const wood = kind !== "cave";
    const f = wood ? 210 : 96;
    this._tone(f, wood ? 0.2 : 0.28, { type: "triangle", gain: 0.2, slideTo: f * 0.55 });
    this._tone(f * 2.1, 0.1, { type: "sine", gain: wood ? 0.09 : 0.03, slideTo: f * 1.4 });
    this._noise(wood ? 0.09 : 0.2, { type: "lowpass", freq: wood ? 1100 : 420, gain: 0.18, attack: 0.002 });
    if (!wood) this._noise(0.5, { type: "lowpass", freq: 700, gain: 0.05, attack: 0.05, delay: 0.06 }); // the cave answers
  }

  /** a mill paddle catches the stone: a wet wooden slap and the wheel groaning */
  paddleWhack(flung = true) {
    this._noise(0.16, { type: "lowpass", freq: 1500, gain: 0.3, attack: 0.002, slideTo: 400 });
    this._tone(150, 0.18, { type: "square", gain: 0.12, slideTo: flung ? 260 : 80 });
    if (flung) this._tone(300, 0.24, { type: "triangle", gain: 0.12, slideTo: 620, delay: 0.03 });
    this._tone(70, 0.5, { type: "sine", gain: 0.06, slideTo: 58, delay: 0.02 }); // the axle takes it
  }

  /** going over a lip: the drop swallows the stone for a moment */
  plunge(drop = 5) {
    const d = Math.min(1, drop / 8);
    this._noise(0.7, { type: "lowpass", freq: 900 + 500 * d, gain: 0.16 + 0.12 * d, attack: 0.03, slideTo: 260 });
    this._tone(90, 0.5, { type: "sine", gain: 0.1 * d, slideTo: 45, delay: 0.05 });
  }

  /**
   * A stone put down on ice. Not a splash and not a thud: a long bright hiss
   * with a ringing edge on it, because a frozen sheet is a drumhead. Faster
   * arrivals hiss higher and hold on longer, which is the only cue you get for
   * how far this one is going to run.
   */
  iceSlide(speed = 10) {
    const s = Math.min(1, speed / 18);
    this._noise(0.5 + s * 1.1, {
      type: "bandpass", freq: 2600 + 1800 * s, gain: 0.1 + 0.1 * s, attack: 0.02, slideTo: 900,
    });
    this._tone(520 + 260 * s, 0.5, { type: "sine", gain: 0.05, slideTo: 300 });
    this._tone(1400, 0.22, { type: "triangle", gain: 0.05 * s, slideTo: 1900 }); // the sheet answering
  }

  /** ploughing into reeds: a wet rustle that eats the stone's pace */
  weedDrag(thick = 1) {
    this._noise(0.34 + thick * 0.2, {
      type: "highpass", freq: 1500, gain: 0.1 + 0.08 * thick, attack: 0.01, slideTo: 600,
    });
    this._noise(0.18, { type: "lowpass", freq: 500, gain: 0.08 * thick, attack: 0.005 });
  }
  ...
  /** the tee light changing on the way to green; the pitch climbs with the count */
  startLight(i) {
    this._tone(392 + i * 42, 0.11, { type: "square", gain: 0.1 });
  }

  /** green. Deliberately nothing like the count that led up to it */
  startGo() {
    this._tone(880, 0.3, { type: "square", gain: 0.15 });
    this._tone(1320, 0.26, { type: "triangle", gain: 0.09, delay: 0.03 });
  }
